...
/**
 * Discover collection card — original OpenSea-style tall card.
 */
export function FeaturedCollectionCard({ collection, rank, className }: Props) {
  ...
  const volShare = (
    (collection.volume24h / Math.max(collection.volumeTotal, 1)) *
    100
  ).toFixed(1)
  ...
  return (
    <Link
      ...
      className={clsx(
        'group relative flex flex-col shrink-0 w-[min(72vw,240px)] sm:w-[260px] lg:w-[272px]',
        'rounded-2xl border border-edge bg-surface overflow-hidden',
        'transition-all duration-300',
        'hover:-translate-y-1 hover:border-hood/50 hover:shadow-[0_12px_40px_rgba(0,200,5,0.12)]',
        className
      )}
    >
      ...
      <div className="relative px-3.5 -mt-8">
        <div className="relative inline-block">
          <img
            ...
            alt={collection.name}
            referrerPolicy="no-referrer"
            className="w-[60px] h-[60px] rounded-xl border-[3px] border-surface shadow-lg object-cover ring-1 ring-edge bg-surface-2"
          />
          {collection.verified && (
      ...
      <div className="px-3.5 pt-2 pb-3.5 flex-1 flex flex-col">
        ...
        <p className="text-[11px] text-ink-3 line-clamp-1 mt-0.5">
        ...
        <div className="mt-auto pt-3 grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-surface-2 px-2.5 py-1.5">
            <div className="text-[10px] text-ink-3 font-medium uppercase tracking-wide">
              Floor
            </div>
